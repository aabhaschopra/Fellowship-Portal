...
let colorArr = ["pink", "blue", "green", "purple"]; 
...
if (localStorage.getItem("allTask")) {
    ...
    for (let i = 0; i < taskArr.length; i++){
        let { id, color, task,date,time } = taskArr[i];
        createTask(color,task,time,date,false,id); //displaying task
    }
}
...
plusBtn.addEventListener("click", createModal);
    ...
    // b. textArea press Enter => modal vanishes + creates task ticket(createTask())
        ...
        textArea.addEventListener('keydown', function (e) {
            if ((e.key == "Enter") && (textArea.value != " " ) ){
                ...
                createTask(ccolor,textArea.value,timeControl.value,dateControl.value,true); // true => ui se creating task tickets + no id sent
            }
        })  
    ...
    function createTask(clr,text,time,date,flag,id){
        ...
        taskContainer.innerHTML = `
            ...
            <div class="rate">
                <input class="check" type="radio" id="star5" name="rate" value="5" />
                <label for="star5" title="text">5 stars</label>
                <input class="check" type="radio" id="star4" name="rate" value="4" />
                <label class="check" for="star4" title="text">4 stars</label>
                <input type="radio" id="star3" name="rate" value="3" />
                <label for="star3" title="text">3 stars</label>
                <input class="check" type="radio" id="star2" name="rate" value="2" />
                <label for="star2" title="text">2 stars</label>
                <input class="check" type="radio" id="star1" name="rate" value="1" />
                <label for="star1" title="text">1 star</label>
            </div>
            <div>
                <label><input type="checkbox">Mark Attendance</label>
            </div>`
